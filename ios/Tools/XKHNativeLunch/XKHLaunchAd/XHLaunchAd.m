...
+(void)soundAction:(NSInteger)inter{
    
    if (inter == 1) {
        //点击了 把声音关闭
        
        [[XHLaunchAd shareLaunchAd] soundButtonClick:@"1"];
        
    }else{
        //把声音再打开
        [[XHLaunchAd shareLaunchAd] soundButtonClick:@"0"];
...
//    if(configuration.scalingMode) self.adVideoView.adVideoScalingMode = configuration.scalingMode;
    ...
    if(configuration.videoNameOrURLString.xh_isURLString)
    {
        ...
        if(pathURL)
        {
            ...
            [self.adVideoView updatePlayerWithURL:pathURL];
...
-(void)soundButtonClick:(NSString*)str
{
    //点击默认播放
  
    if ([str isEqualToString:@"1"]) {
        _adVideoView.player.volume = 1;
      
    }else{
        _adVideoView.player.volume = 0;
...
-(void)remove{
    ...
    if(_adVideoView.player) _adVideoView.player = nil;
