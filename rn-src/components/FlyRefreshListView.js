 ...
 *
 *
 * @providesModule FlyRefreshListView
 * @flow
 *
 */
'use strict';
...
const FlyRefreshListView = React.createClass({
  _dataSource: [],
  _page: 1,
    ...
    if (!this.state.lastError.isReloadError) {
      ...
      if (this.state.noMore) {
        if (this.props.text) {
          ...
          let imgView =  (
            <FlyImage source={'./imgs/investmentRecord/noData.png'} width={FlyDimensions.deviceDim.width * 0.3}/>
          );
          let btnView = this.props.btnText ? (
            <TouchableOpacity style={styles.btnText} onPress={this.props.btnonPress}>
              <Text style={{color:FlyColors.baseColor,fontSize:FlyDimensions.fontSizeXl}}>{this.props.btnText}
              </Text>
            </TouchableOpacity>
          ) : null;
          bottomView = (
            <View style={this.props.btnText ? styles.noMoreStyle : styles.noMoreStyle1}>
          ...
          bottomView = (
            <View style={styles.noData}>
              <FlyImage source={'./imgs/investmentRecord/noData1.png'} width={FlyDimensions.deviceDim.width * 0.3}/>
              <Text style={[styles.noMoreText,{marginTop:15}]}>{'暂无数据'}
              </Text>
            </View>
          )
        }
...
var styles = StyleSheet.create({
  ...
  noData:{
    ...
    justifyContent:'center',
    backgroundColor:FlyColors.baseTextColor3,
    marginTop:FlyDimensions.deviceDim.width * 0.3
  },
  btnText:{
    borderWidth:1,
    borderColor:'#DDDDDD',
...
module.exports = connect(select,null,null,{withRef:true})(FlyRefreshListView);
