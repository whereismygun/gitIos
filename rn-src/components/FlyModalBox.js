...
import React from 'react';
 ...
 var FlyModalBox = React.createClass({
   ...
   calculateModalPosition: function(containerHeight, containerWidth) {
     var position = 0;
      if (this.props.positionNumber) {
        position=this.props.positionNumber
        return position;
      }
     ...
     if (this.props.backdrop) {
       backdrop = (
         <TouchableWithoutFeedback onPress={this.props.backdropPressToClose ? this.close : null}>
           <Animated.View style={[styles.absolute, size, {opacity: this.state.backdropOpacity}, this.props.modalStyle]}>
   ...
   render: function() {
     ...
     return (
       <View style={[styles.transparent, styles.absolute, this.props.modalStyle]} pointerEvents={'box-none'} onLayout={this.onContainerLayout}>
         {backdrop}
         <Animated.View
